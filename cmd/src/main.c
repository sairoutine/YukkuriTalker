...
int main(int argc,char **argv){
	...
	// Phont ファイルの読み込み
	unsigned char *pPhont;
	FILE *pfi=fopen("./phont/aq_yukkuri.phont","rb");
	if(pfi==NULL){
		fprintf(stderr,"couldn't open input file %s\n", "aq_yukkuri.phont");
		return -2;
	}
	fseek(pfi,0,SEEK_END);
	int pfsize=ftell(pfi);
	pPhont=malloc(pfsize);
	fseek(pfi,0,SEEK_SET);

	fread(pPhont,pfsize,1,pfi);
	fclose(pfi);
	...
	int size;
	unsigned char *wav=AquesTalk2_Synthe_Utf8(text,speed,&size, pPhont);
	free(text);
